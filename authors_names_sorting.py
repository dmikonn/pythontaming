...
)  # ordered dictionary would be useful for sorting later
...
for author in authors:
    splited_a = author.split()  # splits list of authors into singular records
    x = 0
    counted_splited_a = []
    for (
        sp_a
        ...
        splited_a
    ):  # splits records of authors, measuring them, uppers letters for further sorting, and gather into arrays
        counted_splited_a.append(sp_a.upper())
        x = x + len(sp_a)
    if x == 4:  # fills dictionary of authors and the crucial sorting variables
